...
Copyright 2007, 2008 David Letscher and Michael H. Goldwasser

Go to www.cs1graphics.org for more information.

Version 1.1:   05/25/2008
Version 1.1.1: 01/22/2010  (bug fix in Drawable.flip)
Version 1.1.2: 03/07/2010  (bug fix in Text.getFontSize, and Point.normalize)
Version 1.1.3: 03/30/2010  (bug fix with Path.addPoint and Spline.addPoint, and Image.draw)
Version 1.1.4: 04/19/2010  (bug fix removing inadvertent tabs)

Modifications by Otfried Cheong for KAIST CS101.
"""
...
_debug = 1                # can be 0, 1, 2 or 3

_enableEvents = True      # If False event handling is turned off (other that waits)
                          # and graphics manager completely render scene before returning
                          # control to the user

_mathMode = False         # If True origin if moved to the lower left hand corner of screen

_underlyingLibrary = 'Tkinter'
                          # Current the only supported option in 'Tkinter'
...
class _GraphicsManager:
  def __init__(self):
    ...
    self._waitingTriggers = set()       # All canvases and drawables currently waiting
  ...
  def processCommands(self):
    ...
    try:
      while self._running and self._commandQueue.qsize() > 0:
        ...
        try:
          self.processCommand(command, blocking)
        except GraphicsError, ge:
          print ge.message
          if not ge._recoverable:
            self._running = False
            raise ge
          elif blocking:
  ...
  def processCommand(self, command, blocking):
    ...
    if command[0] == 'create canvas':
      ...
      self._transform[((command[1], None), )] = command[7]
      ...
      self._transformationChain = (command[1]._transform, )
    ...
    elif command[0] == 'draw spline':
      chain = self._currentChain
      if self._underlyingObject.has_key(chain):
        self._underlyingObject[chain].update(self._transformationChain[-1], self._force)
      else:
        # Create path in the correct position and add it to the canvas
        self._underlyingObject[self._currentChain] = _RenderedSpline( command[1], self._currentCanvas, self._transformationChain[-1], command[1].getPoints() )
        
    elif command[0] == 'draw closed spline':
      chain = self._currentChain
      if self._underlyingObject.has_key(chain):
        self._underlyingObject[chain].update(self._transformationChain[-1], self._force)
      else:
        # Create polygon in the correct position and add it to the canvas
        self._underlyingObject[self._currentChain] = _RenderedClosedSpline( command[1], self._currentCanvas, self._transformationChain[-1], command[1].getPoints() )
    ...
    elif command[0] == 'draw image':
      chain = self._currentChain
      if self._underlyingObject.has_key(chain):
        self._underlyingObject[chain].update(self._transformationChain[-1], self._force)
      else:
        # Create polygon in the correct position and add it to the canvas
        self._underlyingObject[self._currentChain] = _RenderedImage( command[1], self._currentCanvas, self._transformationChain[-1])

    elif command[0] == 'load image':
      try:
        if not _pilAvailable:
          image = _Tkinter.PhotoImage(command[1])
        else:
          s = command[1]
          if s[:7] == "base64:":
            data = _base64.b64decode(s[7:])
            r = _cStringIO.StringIO(data)
            image = _Image.open(r).convert('RGBA')
          else:
            image = _Image.open(command[1]).convert('RGBA')
        self._releaseQueue.put(None)
      except:
        raise GraphicsError('Unable to open image file', True)
    ...
    if _enableEvents:
      if self._handlers.has_key(shape):
        if callback not in self._handlers[shape]:
          ...
          raise ValueError('Shape cannot handle events')
      else:
        self._handlers[shape] = [callback]
    else:
      raise GraphicsError('Cannot handle events in single thread mode.', True)
  ...
  def loadImage(self, filename):
    _graphicsManager.addCommandToQueue(('load image',filename),True)

class GraphicsError(StandardError):
...
class Point(object):
  ...
  def normalize(self):
    ...
    if mag > 0:
      self.scale(1./mag)    # ensure floating point arithmetic
...
class _Transformation(object):
  ...
  def scaleAndTranslate(self):
    if self._matrix[0] == self._matrix[3] and self._matrix[1] == self._matrix[2] == 0:
      return True
    return False

  def diagonalAndTranslate(self):
    if self._matrix[1] == self._matrix[2] == 0:
      return True
    return False

  def translateOnly(self):
    if self._matrix[1] == self._matrix[2] == 0 and self._matrix[0] == self._matrix[3] == 1:
      return True
    return False
...
class Event(object):
  ...
  def getButton(self):
    """Return number of the mouse button that caused the mouse event."""
    return self._button
...
class _EventTrigger(object):
  ...
  def wait(self):
    ...
    _graphicsManager._waitingTriggers.add((self,rh))
    self.addHandler(rh)
    lock.acquire()
    self.removeHandler(rh)
    _graphicsManager._waitingTriggers.remove((self,rh))
    return rh._event
...
class _AnimationThread(_threading.Thread):
  def __init__(self, canvas, base, extension, interval=None):
    _threading.Thread.__init__(self)
    self._canvas = canvas
    self._base = base
  ...
  def saveFrame(self):
    filename = self._base + str(self._frame).rjust(5,'0') + self._extension
...
class Canvas(_GraphicsContainer, _EventTrigger):
  ...
  def __init__(self, w=200, h=200, bgColor=None, title='Graphics canvas', autoRefresh=True):
    ...
    _graphicsManager.addCommandToQueue(('create canvas', self, w, h, self._backgroundColor, title, autoRefresh, self._transform))
  ...
  def close(self):
    ...
    """
    if self._canvasOpen:
      # First release any waits on the canvas or any drawable in it
      waiting = set()
      for w in _graphicsManager._waitingTriggers:
        if w[0] == self or self in _graphicsManager._canvases.get(w[0], set()):
          waiting.add(w[1])

      for w in waiting:
        e = Event()
        e._eventType = 'canvas close'
        w.handle(e)
  ...
  def open(self):
    ...
    if not self._canvasOpen:
      ...
      if not self._canvas:
        _graphicsManager.addCommandToQueue(('create canvas', self, self._width, self._height, self._backgroundColor, self._title, self._autoRefresh, self._transform))
      else:
  ...
  def setViewWindow(self, lowerLeft, upperRight):
  ...
  def zoom(self, factor, center=None):
    if not center:
      center = Point(self.getWidth()/2., self.getHeight()/2.)

    self._transform = self._transform * _Transformation( (factor,0,0,factor,
        self.getWidth()/2 - factor*center.getX(), self.getHeight()/2 - factor*center.getY()) )
  ...
  def rotate(self, angle, center=None):
    ...
    translation = _Transformation( (1,0,0,1,center.getX(),center.getY()) )
    angle = -_math.pi*angle/180.
  ...
  def saveToFile(self, filename):
    ...
    if not _pilAvailable:
      if ext not in ['eps','ps']:
        raise ValueError('Unsupported file type')
    else:
      if ext not in ['eps','gif','jpg','jpeg','png']:
        raise ValueError('Unsupported file type')   
    ...
    if ext in ['eps','EPS','ps','PS']:
      epsFilename = filename
    else:
      fd, epsFilename = _tempfile.mkstemp('.eps')
      _os.close(fd)

    _graphicsManager.addCommandToQueue(('save to file', self, epsFilename), True)

    if ext not in ['eps','EPS']:  # Use PIL to convert
  ...
  def beginAnimation(self, baseFilename, extension, interval=None):
    """Begin an animation that will save the canvas as a sequence of image files.

    Each frame of the animation will be saved with a filename staring with
    baseFilename followed by the frame number and then the given extension.
    for saveToFile for the supported file types.
    """
    if self._animation:
      raise StandardError('An animation is already running, cannot start another')
    if interval:
      if not isinstance(interval, int) and not isinstance(interval, float):
        raise TypeError('Time interval must be numeric')
      if interval <= 0:
        raise ValueError('Time interval must be positive')

    self._animation = _AnimationThread(self, baseFilename, extension, interval)
    if interval:
      self._animation.start()
    else:
      self._animation.saveFrame()

  def createFrame(self):
    """Save a frame to a running animation."""
    if not self._animation:
      raise StandardError('An animation is not currently running')
    self._animation.saveFrame()
...
class Shape(Drawable):
  ...
  def scale(self, factor):
    self._borderWidth *= factor
    Drawable.scale(self, factor)
...
class Ellipse(FillableShape):
  ...
  def __init__(self, width=10, height=10, centerPt=None):
    """Construct a new instance of Circle.

    width    the ellipse's width (default 10)
    height   the ellipse's height (default 10)
    centerPt  a Point representing the placement of the circle's center
                (default Point(0,0) )
    ...
    if width <= 0 or height < 0:
      raise ValueError("The ellipse's width and height must be positive.")
    if centerPt and not isinstance(centerPt,Point):
      raise TypeError("The ellipse's center must be specified as a Point")

    FillableShape.__init__(self) # intentionally not sending center
    if not centerPt:
      centerPt = Point()
    self._transform = _Transformation( (.5*width,0.,0.,.5*height,centerPt.getX(),centerPt.getY()) )
  ...
  def setWidth(self, w):
    ...
    if w <= 0:
      raise ValueError("The ellipse's width must be positive.")
...
class Spline(Shape):
  ...
  def clearPoints(self):
    """Remove all points, setting this back to an empty Spline."""
    self._points = list()
    self._objectChanged(True,False,False)
  ...
  def getPoint(self, index):
    ...
    Subsequently mutating that copy has no effect on the Spline.
    """
    if not isinstance(index,int):
      raise TypeError('index must be an integer')
    try:
  ...
  def getPoints(self):
    """Return a list of Point instances that are copies of the points on the Spline."""
    return list(self._points)
    
  def _draw(self):
    self._beginDraw()

    _graphicsManager.addCommandToQueue(('draw spline', self))

    self._completeDraw()

class ClosedSpline(Spline,FillableShape):
  ...
  def _draw(self):
    ...
    _graphicsManager.addCommandToQueue(('draw closed spline', self))
...
class Image(Drawable):
  def __init__(self, filename):
    ...
    _graphicsManager.loadImage(filename) # load it to see if its possible
  ...
  def rotate(self,angle):
    """Not yet implemented."""
    raise NotImplementedError('rotating an image is not yet implemented')
  ...
  def scale(self, factor):
    ...
    """
    if not _pilAvailable:
      raise NotImplementedError('scaling an image is not yet implemented unless Python Imaging Library is installed')
    Drawable.scale(self, factor)
  ...
  def stretch(self,xFactor,yFactor,angle=0):
    ...
    if not _pilAvailable:
      ...
      raise NotImplementedError('streching can only be done along the corrodinate axes.')
    Drawable.stretch(xFactor,yFactor)
  def flip(self,angle=0):
    """Not yet implemented."""
    raise NotImplementedError('fliping an image is not yet implemented')

  def shear(self, shear, angle=0):
    """Not yet implemented."""
    raise NotImplementedError('shearing an image is not yet implemented')
  ...
  try:
    ...
    class _TkGraphicsManager(_GraphicsManager):
      ...
      def _closeAll(self):
        ...
        for canvas in self._openCanvases:
          canvas._canvas._tkWin.withdraw()
        self._openCanvases = set()
        _tkroot = None
      ...
      def update(self, transform, force=True):
        if _graphicsManager._needsUpdatingInfo[1] or force:
          if self._drawable._borderWidth > 0:
            w = transform.scale()/self._drawable._transform.scale()*self._drawable._borderWidth
            self._canvas._canvas._canvas.itemconfigure(self._object, fill=getTkColor(self._drawable._fillColor), outline=getTkColor(self._drawable._borderColor), width=w)
          else:
    ...
    class _RenderedPath(_RenderedShape):
      def __init__(self, drawable, canvas, transform, points):
        ...
        if len(points) > 1:
          w = transform.scale()/self._drawable._transform.scale()*self._drawable.getBorderWidth()
          tkPts = [(transform.image(p).getX(),transform.image(p).getY()) for p in points]
          self._object = canvas._canvas._canvas.create_line(tkPts,fill=getTkColor(self._drawable._borderColor),  width=w)
      ...
      def update(self, transform, force=True):
        if _graphicsManager._needsUpdatingInfo[0] or force:
          w = transform.scale()/self._drawable._transform.scale()*self._drawable.getBorderWidth()
          self._canvas._canvas._canvas.itemconfigure(self._object, fill=getTkColor(self._drawable._borderColor), width=w)
    ...
    class _RenderedSpline(_RenderedShape):
      def __init__(self, drawable, canvas, transform, points):
        _RenderedShape.__init__(self, drawable, canvas)
        ...
        if len(points) > 1:
          w = transform.scale()/self._drawable._transform.scale()*self._drawable.getBorderWidth()
          tkPts = [(transform.image(p).getX(),transform.image(p).getY()) for p in points]
          self._object = canvas._canvas._canvas.create_line(tkPts,fill=getTkColor(self._drawable._borderColor),  width=w, smooth=1)
      ...
      def update(self, transform, force=True):
        if _graphicsManager._needsUpdatingInfo[0] or force:
          self._canvas._canvas._canvas.itemconfigure(self._object, fill=getTkColor(self._drawable._borderColor),smooth=1)
    ...
    class _RenderedClosedSpline(_RenderedFillableShape):
      def __init__(self, drawable, canvas, transform, points):
        _RenderedFillableShape.__init__(self, drawable, canvas)

        if len(points) > 2:
          tkPts = [(transform.image(p).getX(),transform.image(p).getY()) for p in points]
          self._object = canvas._canvas._canvas.create_polygon(tkPts,smooth=1)
      ...
      def update(self, transform, force=True):
        if _graphicsManager._needsUpdatingInfo[0]:
          if len(self._drawable.getPoints()) > 2:
            ...
            _RenderedFillableShape.update(self, transform, force)
            self._canvas._canvas._canvas.itemconfigure(self._object, smooth=1) 
    ...
    class _RenderedText(_RenderedDrawable):
      ...
      def update(self, transform, force=True):
        ...
        if _graphicsManager._needsUpdatingInfo[1]:
          self._canvas._canvas._canvas.itemconfigure(self._object, font=('Helvetica', int(self._drawable._size*scale), 'normal'), fill=getTkColor(self._drawable._color), text=self._drawable._text)
    ...
    class _RenderedImage(_RenderedDrawable):
      def __init__(self, drawable, canvas, transform):
        ...
        if drawable._filename[:7] == "base64:":
          data = _base64.b64decode(drawable._filename[7:])
          r = _cStringIO.StringIO(data)
          self._baseImage = _Image.open(r).convert('RGBA')
        else:
          self._baseImage = _Image.open(drawable._filename).convert('RGBA') 
 
        if _pilAvailable:
          if transform.diagonalAndTranslate() and transform._matrix[0] > 0 and transform._matrix[3] > 0:
            imageCopy = self._baseImage.resize( (int(round(self._baseImage.size[0] * transform._matrix[0])), int(round(self._baseImage.size[1] * transform._matrix[3]))))
            self._image = _ImageTk.PhotoImage(imageCopy)
            self._canvas._canvas._canvas.itemconfigure(self._object, image=self._image,anchor='center')
          else:
            raise GraphicsError('Images cannot be rotated, flipped or sheared', True)
        else:
          if not transform.translateOnly():
            raise GraphicsError('Images can only be tranlated unless Python imaging library is installed', True)
          self._image = _Tkinter.PhotoImage(file=drawable._filename)
        self._object = canvas._canvas._canvas.create_image(center.get(), image=self._image,anchor='center')
...
elif _underlyingLibrary == 'PIL':
  pass
else:
  print 'No valid underlying graphics library selected'
